...
<head>
    ...
    <title>{{ config('app.name', 'CyberForensics AI') }} - SOC Dashboard</title>
        ...
        <aside class="w-64 glass-panel border-r border-slate-800 flex flex-col justify-between shrink-0">
            <div>
                ...
                <nav class="p-4 space-y-1">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('dashboard') ? 'bg-red-500/10 text-red-400 border-l-2 border-red-500 font-semibold' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200' }}">
                        <i class="fa-solid fa-chart-line w-5 text-center text-base"></i>
                        SOC Dashboard
                    </a>
                    
                    @if(Auth::user() && Auth::user()->isAdmin())
                        <a href="{{ route('admin.users') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 {{ request()->routeIs('admin.users') ? 'bg-red-500/10 text-red-400 border-l-2 border-red-500 font-semibold' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200' }}">
            ...
            <header class="h-16 glass-panel border-b border-slate-800 flex items-center justify-between px-8 z-10">
                <div class="flex items-center gap-4">
                    <h2 class="text-lg font-semibold tracking-wide text-slate-200">
                        @yield('header_title', 'Incident Analysis Center')
                    </h2>
                </div>
                <div class="flex items-center gap-4">
                    <!-- Status widget -->
