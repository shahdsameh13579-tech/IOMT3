...
<body>
    ...
    <div class="header">
        ...
        <div class="meta-info">
            <strong>Case ID:</strong> #{{ $analysis->id }}<br>
            <strong>Date:</strong> {{ $analysis->created_at->format('Y-m-d H:i:s') }}<br>
            <strong>Investigator:</strong> {{ $analysis->log->user->name }}
        </div>
        <div class="clear"></div>
    </div>
    ...
    <div class="summary-card">
        <p>
            An automated forensic analysis has been performed on the web server log package <strong>{{ $analysis->log->file_name }}</strong>. 
            ...
            Out of <strong>{{ number_format($analysis->total_entries) }}</strong> parsed requests, 
            <strong>{{ number_format($analysis->total_findings) }}</strong> potential attack instances were classified as malicious.
        </p>
    </div>
    ...
    <table class="metrics-table">
        <tr>
            <td>
                ...
                <div class="val" style="color: #ef4444;">{{ $analysis->total_findings }}</div>
            </td>
            <td>
                <div class="lbl">Zero-Day Rate</div>
                <div class="val" style="color: #a855f7;">{{ $analysis->zero_day_rate }}%</div>
            </td>
            <td>
                <div class="lbl">Model Accuracy</div>
                <div class="val" style="color: #10b981;">{{ round($analysis->accuracy * 100, 1) }}%</div>
            </td>
            <td>
                <div class="lbl">Log Entries</div>
                <div class="val">{{ number_format($analysis->total_entries) }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="details-table">
        ...
        <tbody>
            @foreach($attackDistribution as $type => $count)
                <tr>
                    <td style="font-weight: bold; color: #475569;">{{ $type }}</td>
                    <td style="text-align: right; font-weight: bold; font-family: monospace;">{{ $count }}</td>
                </tr>
            @endforeach
            @if(empty($attackDistribution))
                <tr>
    ...
    <ul class="recommendation-list">
        @if(isset($attackDistribution['SQLi']) && $attackDistribution['SQLi'] > 0)
        ...
        @endif
        @if(isset($attackDistribution['XSS']) && $attackDistribution['XSS'] > 0)
        ...
        @endif
        @if(isset($attackDistribution['Directory Traversal']) && $attackDistribution['Directory Traversal'] > 0)
        ...
        @endif
        @if(isset($attackDistribution['Command Injection']) && $attackDistribution['Command Injection'] > 0)
        ...
        @endif
        @if(isset($attackDistribution['Credential Stuffing']) && $attackDistribution['Credential Stuffing'] > 0)
        ...
        @endif
        @if(isset($attackDistribution['Vulnerability Scan']) && $attackDistribution['Vulnerability Scan'] > 0)
        ...
        @endif
        @if(isset($attackDistribution['Zero-Day Anomaly']) && $attackDistribution['Zero-Day Anomaly'] > 0)
    ...
    <div class="header">
        ...
        <div class="meta-info">
            <strong>Case ID:</strong> #{{ $analysis->id }}<br>
            <strong>Showing Top:</strong> {{ min(100, $findings->count()) }} findings
        </div>
        <div class="clear"></div>
    </div>
    ...
    <table class="details-table">
        ...
        <tbody>
            @forelse($findings as $finding)
                <tr>
                    <td style="font-size: 9.5px; font-family: monospace;">{{ $finding->timestamp }}</td>
                    <td style="font-weight: bold; font-family: monospace;">{{ $finding->ip }}</td>
                    ...
                    <td style="text-align: center; font-weight: bold; font-family: monospace; color: {{ $finding->status >= 400 ? '#ef4444' : '#10b981' }}">{{ $finding->status }}</td>
                </tr>
            @empty
                <tr>
